...
/*=========================================================
  ...
  $Id: reflections.m 65626 2022-03-26 06:25:17Z don $
...
{ If r is a reflection, return true, a root, and a coroot }
  flag, reftype, root, coroot := isPseudoReflection_(r);
...
{The imprimitive complex reflection (Shephard-Todd) group G(m,p,n)}
  require IsDivisibleBy(m,p): "second argument must divide first";
  if m eq 1 and n eq 1 then return sub<GL(1,Rationals())|>; end if;
...
{ The primitive complex reflection group of type X_n}
  ...
  else // this branch takes care of types E_n and F_4
    return ShephardTodd( ShephardToddNumber( X, n ) : NumFld := NumFld );
  end if;
end intrinsic;


// Reflection groups (moved from GrpCox.m)
// -------------------------
...
{The reflection group with the given simple roots, coroots, and orders}
  ...
  require BaseRing( roots ) eq BaseRing( coroots ) : 
    "The roots and coroots must be over the same ring";
  require Universe( orders ) eq Integers() and forall{ m : m in orders | m ge 2 } : 
    "The orders must be integers greater than two";

  // ensure that (A[i],B[i])=2
  for i in [1..Nrows(roots)] do
    c := (roots[i],coroots[i]);
    if c ne 2 then MultiplyRow( ~coroots, 2/c, i ); end if;
  end for;
...
{ Test if the generators of G are reflections.  Set Strict to false
  ...
  if assigned G`IsReflectionGroup then return G`IsReflectionGroup; end if;
...
isParallel := function( u, v )
  d := Depth(u);
  return d eq 0 or v[d]*u eq u[d]*v;
end function;
...
{True iff w is a reflection}
  ...
  if ret then
    rts := Roots( W );
    ok := exists(r){ r : r in [1..#rts] | isParallel( v, rts[r] ) };
...
// IsRealReflectionGroup(G :: GrpMat) is defined in GrpCox.m
