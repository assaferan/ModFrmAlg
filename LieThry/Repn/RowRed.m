...
import "Repn.m": isPara, inverseops, 
    leftActionOps, rightActionOpsfunc, rightActionOps,
    rSpace;
...
WeightBase := function( rho )
  ...
  for i in [2..#B] do
    Orbit, Action := WeightOrbit( RootDatum(G), B[i] : J := J[i]);
...
findPivotInRowVariant1 := function( rho, X, i, remaining
    ...
    if exists(j){ j : j in remaining | ( im*(rho`WeightBase_M), (rho`WeightBase_vectOrbit)[j] ) ne 0 } then
      return j, g;
    end if;
    return false, _;  
end function;
...
weylPart := procedure(rho, ws, ~X, ~w, u, ~wdot, v, h : Vectors := false, Sides := false)
    G := Domain(rho);
    wdot := elt< G | &*[ WeylGroup(G) | wi : wi in Reverse(ws) ] >;
...
rowReduction := function( rho : Sides:=false, Vectors:=false,
  ...
  return function( X )
    ...
    unipotentParts( rho, clearrow, clearcol, ~X, ~u, ~v, ~ws, ~excp
        : Vectors := Vectors, Sides := Sides);
    if (excp cmpne false) then return false, excp; end if;
