...
import "lattice.m" : SteinitzGram, LocalBasis;
import "../helper.m" : MVM;
...
function BuildNeighbor(nProc : BeCareful := true, UseLLL := true,
		Special := false)
	...
	// If we don't require LLL, return what we have.
	if not UseLLL then return nLat; end if;

	// Perform LLL on the basis for increased performance.
	nLat2 := LLL(nLat : Proof := false);

	// TODO: Come up with a better way to ensure that LLL doesn't move
	//  lattices out of the proper isometry class when the isogeny type is
	//  specified as "SO".

	// TODO: Also, make this work.
	if Special then
		_, sigma := IsIsometric(nLat, nLat2);
		if Determinant(sigma) eq -1 then
			basis := Basis(nLat2);
			basis := Matrix([ basis[2], basis[1] ] cat
				[ basis[i] : i in [3..#basis] ]);
			nLat2 := LatticeWithBasis(basis,
				InnerProductMatrix(nLat2));
		end if;
	end if;
