...
// remove these if herm.m is included in the package:
...
{A Z-basis for the module M}
  ...
  P:= PseudoBasis(M);
  // JEFF FIXED THIS (11/9/2015)
  //B := [ b * p[2] : b in AbsoluteBasis(NumberField(Order(p[1]))), p in P ];
...
function AutoIsomDec(L1, L2)
  ...
  if not assigned L1`Auto then
    ...
    BF:= RowSubmatrix(B1, Idx)^-1;
    Gens:= [];
    for g in GG do
      a:= RowSubmatrix(g, Idx);
      Append(~Gens, BF * Matrix([ &+[ B1[j] * a[i,j] : j in [1..m] | a[i,j] ne 0 ] : i in [1..d] ]));
    end for;
    A:= MatrixGroup< d, BaseRing(BF) | Gens >;
    L1`Auto:= MatrixGroup< d, BaseRing(BF) | Gens >;
...
intrinsic AutomorphismGroup(L::LatMod : Decomposition:= 0) -> GrpMat
...
/*      AA:= L`Auto;
      delete L`Auto;
      A:= AutomorphismGroup(L: Decomposition:= false);
      assert A eq AA;*/
    else
      F, _, B:= Forms(L); 
...
intrinsic IsIsometric(L1::LatMod, L2::LatMod : Check:= false, Decomposition:= 0) -> BoolElt, Mtrx
{Tests if the lattices L1 and L2 are isometric}
  ...
  if Decomposition cmpeq 0 then 
    _, _, _, _, q1:= Forms(L1);
    _, _, _, _, q2:= Forms(L2);
    Decomposition:= Max(q1, q2) ge 10;
    // "Trying decomposition? ", Decomposition;
  end if;

  if Decomposition then
    Decomposition, res1, S:= AutoIsomDec(L1, L2);
    if Decomposition then result:= res1; end if;
  end if;
  if not Decomposition or Check then
    F1, d1, B1, a1:= Forms(L1);
    F2, d2, B2, a2:= Forms(L2);
    error if a1 ne a2 , "Boo-boo: The primitive elements differ!";
  ...
  if Check then
    if Decomposition then assert result eq res1; end if;
    assert S * L1`Form * InternalConjugate(L1, S) eq L2`Form;
    assert Module(Rows(Matrix(Generators(L2`Module)) * S)) eq L1`Module;
  elif not result then return false, _;
  end if;
  return true, S;
end intrinsic;

intrinsic IsSimilar(L1::LatMod, L2::LatMod : Decomposition:= 0) -> BoolElt, Mtrx, FldAlgElt
{Tests if the lattices L1 and L2 are similar}
  ...
  I:= Scale(L1) / Scale(L2);
  if I^m ne Norm(L1) / Norm(L2) then return false, _, _; end if;
  if not orth then
    J:= I;
    I:= I meet Integers(FixedField(L1));
  ...
  for a in aa do
    ...
    ok, x:= IsIsometric(L1, M : Decomposition:= Decomposition);
...
function SetupLatticeInSpace(L, M)
  ...
  LL`Form:= L`Form;
  LL`Module:= M;
  if assigned(L`Definite) then LL`Definite:= L`Definite; end if;
  return LL;
end function;
...
{The maximal sublattices of L that contain pL}
  ...
  for s in Ls do
    ...
    m:= ChangeRing(m, F);
    LL:= SetupLatticeInSpace(L, Module(Rows(m * BM)) + pML);
...
{The sublattices of index p^i in L that contain pL}
  ...
  require AutoOrbits cmpne false: "Set AutoOrbits";
  A:= AutoOrbits cmpeq true select AutomorphismGroup(L) else ChangeRing(AutoOrbits, F);
  BMI:= BM^-1;
  A:= sub< GL(n, k) | [Matrix(k, n, [ x @ h : x in Eltseq(BM * A.i * BMI) ]) : i in [1..Ngens(A)]] >;
  Ls:= [ BasisMatrix(l[2]): l in OrbitsOfSpaces(A, n-i) ];
  ...
  for s in Ls do
    m:= ChangeRing(s, hinv);
    m:= ChangeRing(m, F);
    LL:= SetupLatticeInSpace(L, Module(Rows(m * BM) cat Gens));
    if CallBack cmpne false then
...
//    l:= AutoOrbits select Ls[i] else Next(Ls);
    ...
    LL:= SetupLatticeInSpace(L, ML + Module([< pinv, m*BM >]));
    if CallBack cmpne false then
      keep, cont:= CallBack(LL);
      if keep then Append(~Result, LL); end if;
      if not cont then break; end if;
